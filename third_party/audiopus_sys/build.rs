...
fn link_prebuilt_opus(is_static: bool, installed_lib_directory: &Option<String>) {
    ...
    // Path to prebuilt opus.dll inside the crate. We pick by *target* arch
    // (CARGO_CFG_TARGET_ARCH) instead of build-host arch so cross-compiling
    // from aarch64-pc-windows-msvc to x86_64-pc-windows-msvc works. Upstream
    // 0.1.8 used #[cfg(target_arch)] which evaluates against the host and
    // leaves ARCHITECTURE undefined on aarch64 hosts.
    let target_arch =
        env::var("CARGO_CFG_TARGET_ARCH").unwrap_or_else(|_| "x86_64".to_string());
    let architecture: &str = match target_arch.as_str() {
        "x86" => "x86",
        // aarch64 has no bundled prebuilts; fall back to x64 (used only for
        // cross-builds from ARM hosts to x64 targets).
        _ => "x64",
    };
